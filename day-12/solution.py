...
def count_unique_paths(cave: Dict[str, List[str]]) -> int:
    ...
    return _count_unique_paths_single_entrance(START_NODE_ID, cave, {START_NODE_ID})
...
def count_unique_paths_2(cave: Dict[str, List[str]]) -> int:
    if START_NODE_ID not in cave or END_NODE_ID not in cave:
        raise Exception("One of require nodes (start/end) are missing in given cave representation")
    return _count_unique_paths_2(START_NODE_ID, cave, {START_NODE_ID})


def _count_unique_paths_2(node: str, cave: Dict[str, List[str]], path: Set[str], visited_twice: Optional[str] = None) -> int:
    if node == END_NODE_ID:
        return 1
    ...
    for next_node in cave[node]:
        if next_node.isupper() or next_node not in path:
            count += _count_unique_paths_2(next_node, cave, {*path, next_node}, visited_twice)
        elif next_node != START_NODE_ID and visited_twice is None:
            count += _count_unique_paths_2(next_node, cave, {*path, next_node}, next_node)
...
if __name__ == '__main__':
    ...
    print(count_unique_paths_2(cave))
